...
const handleSubmit = async() => {
    ...
    if(!nameError.value && !slugError.value && !categoryError.value){
        ...
        const {data , status} = await axios.put(import.meta.env.VITE_API_BASE_URL+'/blogs/'+route.params.id ,newFormData , {headers:authHeaders})
        if(status === 200){
            router.push({name:'blogs'})
            toastTypeSuccess('Blog Updated successfully');
        }
        else{
...
const fetchById = async() => {
    const {data , status} = await axios.get(import.meta.env.VITE_API_BASE_URL+'/blogs/'+route.params.id,{headers:authHeaders})
    if(status === 200){
        formData.value.title = data.data.title
        formData.value.slug = data.data.slug
        formData.value.category = data.data.category
        formData.value.description = decode(data.data.description) 
        ...
        <div class="w-full bg-white dark:bg-boxdark p-5 rounded-md shadow">
            <div class="flex flex-col gap-5.5">
                ...
                <div>
                    ...
                    <Editor v-model="formData.description" @onChange="handleChangeData"/>
                </div>
