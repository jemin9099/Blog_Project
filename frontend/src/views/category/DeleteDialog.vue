...
const handleDelete = async() => {
    const {status} =await axios.delete(import.meta.env.VITE_API_BASE_URL+'/category/'+categoryId.value ,{headers:authHeaders})

    if(status === 200){
        toastTypeSuccess('Category Delete successfully');
        emit('refresh')
        dialog.value = false
    }
    else{
        toastTypeError('Error While deleting category')
    }
}
defineExpose({ open })
</script>
<template>
    <div v-if="dialog" class="fixed inset-0 z-9999 flex items-center justify-center bg-gray-500 bg-opacity-50">
        <div class="bg-white dark:bg-boxdark rounded-lg shadow-lg max-w-lg w-full p-6">
            ...
            <h2 class="text-xl text-center font-semibold mb-4">Are you sure you want to delete this category?</h2>
