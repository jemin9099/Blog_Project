...
const fetchCategories = async () => {
    ...
    if(status === 200){
        ...
        toastTypeError('Error while fetching category')
    }
}
...
<template>
    <teleport to='body'>
        <CategoryDialog ref="categoryDialog" @refresh="fetchCategories()"/>
        <DeleteDialog ref="deleteCategory" @refresh="fetchCategories()"/>
    </teleport>
    <DefaultLayout>
        <BreadcrumbDefault :pageTitle="'Category'" />
        <div class="w-full bg-white dark:bg-boxdark p-5 rounded-md shadow">
            <button
                ...
                @click="categoryDialog.open('Add Category')">
                        ...
                        <tbody>
                            <tr v-for="(category , index) in categoryList" :key="index">
                                ...
                                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm flex items-center">
                                    <span class="me-4" @click="categoryDialog.open('Update Category', category)">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="fill-primary " width="26" height="26"
                                        viewBox="0 0 22 22" fill="none">
                                    ...
                                    <span @click="deleteCategory.open(category)">
