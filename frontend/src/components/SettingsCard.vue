...
const userData = JSON.parse(localStorage.getItem('authData') as any)
...
const handleSubmit = async () => {
  ...
  newformData.append('email', formData.value.email);
  newformData.append('bio', formData.value.bio);
  newformData.append('password', formData.value.password);
  newformData.append('file', selectedImg.value);

  try {
    const response = await axios.put(`${import.meta.env.VITE_API_BASE_URL}/users/${userData.data._id}`, newformData, { headers: authHeaders });
    console.log(response);
    if (response.status === 200) {
      toastTypeSuccess('Profile updated successfully');
    }
  } catch (error) {
    toastTypeError('Error while updating profile');
  }
};
const fetchCurrentUser = async () => {
  const { data: currentUser } = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/users/${userData.data._id}`, {
    headers: authHeaders,
  });
      ...
      <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        ...
        <div class="p-7">
          <form @submit.prevent="handleSubmit">
            ...
            <div class="mb-5.5">
              <label class="mb-3 block text-sm font-medium text-black dark:text-white" for="emailAddress">Email
                Address</label>
              <div class="relative">
                <span class="absolute left-4.5 top-4">
                  <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
            ...
            <!-- Username Section -->
            <div class="mb-5.5">
              <label class="mb-3 block text-sm font-medium text-black dark:text-white" for="Username">Password</label>
              <input v-model="formData.password"
                class="w-full rounded border border-stroke bg-gray py-3 px-4.5 font-normal text-black focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary"
                type="text" name="Username" id="Username" placeholder="Enter Your Password" />
            </div>

            <!-- Bio Section -->
            <div class="mb-5.5">
              <label class="mb-3 block text-sm font-medium text-black dark:text-white" for="bio">BIO</label>
              <div class="relative">
                <span class="absolute left-4.5 top-4">
                  <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
                    <g opacity="0.8" clip-path="url(#clip0_88_10224)">
                      <path fill-rule="evenodd" clip-rule="evenodd"
                        d="M1.56524 3.23223C2.03408 2.76339 2.66997 2.5 3.33301 2.5H9.16634C9.62658 2.5 9.99967 2.8731 9.99967 3.33333C9.99967 3.79357 9.62658 4.16667 9.16634 4.16667H3.33301C3.11199 4.16667 2.90003 4.25446 2.74375 4.41074C2.58747 4.56702 2.49967 4.77899 2.49967 5V16.6667C2.49967 16.8877 2.58747 17.0996 2.74375 17.2559C2.90003 17.4122 3.11199 17.5 3.33301 17.5H14.9997C15.2207 17.5 15.4326 17.4122 15.5889 17.2559C15.7452 17.0996 15.833 16.8877 15.833 16.6667V10.83hashedPassword33C15.833 10.3731 16.2061 10 16.6663 10C17.1266 10 17.4997 10.3731 17.4997 10.8333V16.6667C17.4997 17.3297 17.2363 17.9656 16.7674 18.4344C16.2986 18.9033 15.6627 19.1667 14.9997 19.1667H3.33301C2.66997 19.1667 2.03408 18.9033 1.56524 18.4344C1.0964 17.9656 0.833008 17.3297 0.833008 16.6667V5C0.833008 4.33696 1.0964 3.70107 1.56524 3.23223Z"
                        fill="" />
                      <path fill-rule="evenodd" clip-rule="evenodd"
          ...
          <form>
            ...
            <div class="mb-4 flex items-center gap-3">
              <div class="h-14 w-14 rounded-full">
                <img :src="previewImg ? previewImg : formData.avatar" alt="User" class="rounded-full" />
              </div>
              <div>
                <span class="mb-1.5 font-medium text-black dark:text-white">Edit your photo</span>
                <span class="flex gap-2.5">
                  You upload or drag your photo into the box below
                </span>
              </div>
            </div>
